...
export function validateNoShellMetacharacters(value, paramName) {
    if (CONTROL_CHARACTERS.test(value)) {
        throw new McpError(ErrorCode.InvalidParams, `${paramName} contains control characters that are not allowed`);
    }
}
function resolvePathForValidation(candidatePath) {
...
function formatAllowedRoots(allowedRoots) {
...
export function parseSemgrepResults(fileContent) {
...
function getSemgrepFindings(results) {
    return Array.isArray(results.results) ? results.results : [];
}
function validateRegistryConfig(configValue) {
    validateNoShellMetacharacters(configValue, 'config');
    if (!REGISTRY_CONFIG_REF.test(configValue)) {
        throw new McpError(ErrorCode.InvalidParams, 'config must be "auto" or a Semgrep registry reference like "p/security"');
    }
    return configValue;
}
export function validateAbsolutePath(pathToValidate, paramName) {
    if (paramName === 'config' && (pathToValidate.startsWith('p/') ||
    ...
    if (!path.isAbsolute(pathToValidate)) {
        throw new McpError(ErrorCode.InvalidParams, `${paramName} must be an absolute path. Received: ${pathToValidate}`);
    }
    const normalizedPath = resolvePathForValidation(pathToValidate);
    if (!path.isAbsolute(normalizedPath)) {
        throw new McpError(ErrorCode.InvalidParams, `${paramName} contains invalid path traversal sequences`);
    }
    const allowedRoots = getAllowedRoots();
    const isWithinAllowedRoots = allowedRoots.some((allowedRoot) => isPathWithinRoot(allowedRoot, normalizedPath));
    if (!isWithinAllowedRoots) {
        throw new McpError(ErrorCode.InvalidParams, `${paramName} must be within an allowed workspace root (${formatAllowedRoots(allowedRoots)})`);
    }
    return normalizedPath;
}
export function validateRuleField(value, paramName, pattern) {
    if (typeof value !== 'string' || !pattern.test(value)) {
        throw new McpError(ErrorCode.InvalidParams, `${paramName} contains invalid characters or exceeds allowed format`);
    }
    return value;
}
export function validateRuleSeverity(value) {
    const upper = String(value).toUpperCase();
    if (!ALLOWED_SEVERITY.has(upper)) {
        throw new McpError(ErrorCode.InvalidParams, `severity must be one of: ${Array.from(ALLOWED_SEVERITY).join(', ')}`);
    }
    return upper;
}
...
class SemgrepServer {
    ...
    async handleScanDirectory(args) {
        ...
        try {
            ...
            const loggedArgs = semgrepArgs.map((arg, idx) => idx > 0 && semgrepArgs[idx - 1] === '--oauth-token' ? '[REDACTED]' : arg);
...
semgrep scan --config=p/ci`;
    ...
    async handleAnalyzeResults(args) {
    ...
    async handleCreateRule(args) {
        if (!args.output_path || !args.pattern || !args.language || !args.message) {
            throw new McpError(ErrorCode.InvalidParams, 'output_path, pattern, language and message are required');
        }
    ...
    async handleFilterResults(args) {
        if (!args.results_file) {
        ...
        try {
            ...
            if (args.severity) {
                filteredResults = filteredResults.filter((finding) => finding.extra?.severity === args.severity);
            }
            if (args.rule_id) {
                filteredResults = filteredResults.filter((finding) => finding.check_id === args.rule_id);
            }
            if (args.path_pattern) {
                const pathRegex = new RegExp(args.path_pattern);
                filteredResults = filteredResults.filter((finding) => pathRegex.test(finding.path));
            }
            if (args.language) {
                filteredResults = filteredResults.filter((finding) => finding.extra?.metadata?.language === args.language);
            }
            if (args.message_pattern) {
                const messageRegex = new RegExp(args.message_pattern);
                filteredResults = filteredResults.filter((finding) => messageRegex.test(finding.extra?.message ?? ''));
            }
            return {
    ...
    async handleExportResults(args) {
        if (!args.results_file || !args.output_file) {
            throw new McpError(ErrorCode.InvalidParams, 'results_file and output_file are required');
        }
        const resultsFile = validateAbsolutePath(args.results_file, 'results_file');
        ...
        try {
            ...
            switch (format) {
                ...
                case 'text':
                default:
                    output = findings.map((r) => `[${r.extra?.severity ?? 'unknown'}] ${r.check_id}\n` +
                        `File: ${r.path}\n` +
    ...
    async handleCompareResults(args) {
        if (!args.old_results || !args.new_results) {
            throw new McpError(ErrorCode.InvalidParams, 'old_results and new_results are required');
        }
        ...
        try {
            ...
            const oldFindings = new Set(oldResults.map((r) => `${r.check_id}:${r.path}:${r.start?.line}:${r.start?.col}`));
            const comparison = {
            ...
            const newKeys = new Set(newResults.map((r) => `${r.check_id}:${r.path}:${r.start?.line}:${r.start?.col}`));
            oldResults.forEach((finding) => {
                const key = `${finding.check_id}:${finding.path}:${finding.start?.line}:${finding.start?.col}`;
                if (!newKeys.has(key)) {
